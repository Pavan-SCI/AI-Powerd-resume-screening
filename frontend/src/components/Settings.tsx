...
export const Settings: React.FC<SettingsProps> = ({
  ...
  if (!isAdmin) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', maxWidth: '800px', flex: 1 }}>
        <div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', maxWidth: '800px', flex: 1 }}>
      ...
      <div className="glass-panel" style={{
        ...
        <p style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', margin: 0 }}>
          Enable Demo Mode to use high-fidelity simulated AI responses <strong>without requiring a Gemini API key</strong>. All users on the platform will use demo responses while this is enabled.
        </p>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginTop: '0.25rem' }}>
          ...
          <span style={{ fontSize: '0.85rem', color: isDemoMode ? 'var(--warning)' : 'var(--text-muted)', fontWeight: 600 }}>
            {isDemoMode ? '✓ Demo Mode is currently ACTIVE' : '✗ Demo Mode is currently INACTIVE'}
          </span>
        </div>
      </div>
        ...
        <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          <div className="form-group" style={{ margin: 0 }}>
            ...
            {isDemoMode && (
              <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)', display: 'block', marginTop: '0.25rem' }}>
                * Disable Demo Mode above to configure a live API key.
              </span>
            )}
          </div>
          ...
          <div className="form-group" style={{ margin: 0 }}>
            ...
            <select
              ...
              disabled={isDemoMode}
            >
              <option value="gemini-2.5-flash">Gemini 2.5 Flash (Recommended — Fast & Precise)</option>
              <option value="gemini-2.5-pro">Gemini 2.5 Pro (Deep Analysis — Slower)</option>
      ...
      <div className="glass-panel" style={{ padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        ...
        <ol style={{
          ...
          <li>Open <a href="https://aistudio.google.com/" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--secondary)', textDecoration: 'underline' }}>Google AI Studio (aistudio.google.com)</a></li>
          ...
          <li>Paste it into the API Key field above and click Save.</li>
        </ol>
        ...
        <div style={{
          ...
          <span>
            <strong>Privacy:</strong> Your API key is stored only in this browser's local storage. All queries are made directly from the browser to Google Gemini API servers — no third-party server processes your key.
          </span>
        </div>
      </div>
    </div>
